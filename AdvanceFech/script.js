...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  data.forEach((item) => {
    list.innerHTML += `<li class="mb-4" data-desc="${item.description}"><a href="detail.html?id=${item.id}">${item.name}</a> <button data-id="${item.id}" class="delete">delete</button> <button data-id="${item.id}" class="edit">edit</button></li>`;
  });
